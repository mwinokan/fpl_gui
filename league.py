...
class League():
	...
	def get_stats(self):
		...
		if self._extra_managers is not None:
			...
			manager_df = pd.concat([manager_df,new_df], ignore_index=True, verify_integrity=False, sort=False)
		...
		if plot:
			...
			with plt.style.context('seaborn'):
				...
				for man in self.managers:
					...
					ax.scatter(man.active_gws,ydata,label=man.name,marker='o',zorder=4)

				# averages
				ax.plot(range(1,self._api._current_gw+1),avgs,linestyle='-',marker=None,color="b",label="Global Average",zorder=3)
				ax.plot(range(1,self._api._current_gw+1),self.average_event_points,linestyle='--',marker=None,label=f"{self.shortname} Average",zorder=1,color=self.colour_str)
