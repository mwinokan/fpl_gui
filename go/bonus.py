...
def main():
	...
	create_bonus_figure(api,players,show=True)

def create_bonus_figure(api,players,show=False):
	gw = api._current_gw
	mout.debugOut(f"create_bonus_figure(gw={gw+1})")
	...
	for i,p in enumerate(players):
		...
		# gwexp = p.expected_points(gw=gw+1,fit_ratio=0.8,use_official=False,debug=False,force=True)
		
		pts_p90 = p.total_points / p.total_minutes * 90
		bps_p90 = p.total_bps / p.total_minutes * 90

		size = p.selected_by/2+5

		if p.position_id == 1:
			xdata_gkp.append(pts_p90)
			ydata_gkp.append(bps_p90)
			tdata_gkp.append(f'{p.name}, {p.team_obj.shortname}, {p.selected_by}%')
			sdata_gkp.append(size)
			udata_gkp.append(f'https://mwinokan.github.io/FPL_GUI/html/player_{p.id}.html')
		elif p.position_id == 2:
			xdata_def.append(pts_p90)
			ydata_def.append(bps_p90)
			tdata_def.append(f'{p.name}, {p.team_obj.shortname}, {p.selected_by}%')
			sdata_def.append(size)
			udata_def.append(f'https://mwinokan.github.io/FPL_GUI/html/player_{p.id}.html')
		elif p.position_id == 3:
			xdata_mid.append(pts_p90)
			ydata_mid.append(bps_p90)
			tdata_mid.append(f'{p.name}, {p.team_obj.shortname}, {p.selected_by}%')
			sdata_mid.append(size)
			udata_mid.append(f'https://mwinokan.github.io/FPL_GUI/html/player_{p.id}.html')
		elif p.position_id == 4:
			xdata_fwd.append(pts_p90)
			ydata_fwd.append(bps_p90)
	...
	fig.update_xaxes(title_text=f"Points per 90")
	# fig.update_xaxes(title_text=f"Official GW{gw} xPts")
	fig.update_yaxes(title_text=f"BPS per 90")
