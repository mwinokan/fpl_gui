...
class FPL_API():
	...
	def __init__(self,offline=False,force_generate_kits=False,quick=True,write_offline_data=False):
		...
		self._team_styles = {
			"Arsenal": {"background-color": "#B7272E","color": "white","accent":"#E5D3B7"},
			"Aston Villa": {"background-color": "#5D1133","color": "#91B6E4","accent":"#F0D147"},
			"Brentford": {"background-color": "#A22621","color": "white","accent":"black"},
			"Bournemouth": {"background-color": "black","color": "#BF3B33","accent":"#B29460"},
			"Brighton": {"background-color": "white","color": "#2253A0","accent":"#1A2561"},
			"Burnley": {"background-color": "#9D3E45","color": "#CCDCF3", "accent":"#FBEA88"},
			"Chelsea": {"background-color": "#051383","color": "white","accent":"PaleGoldenRod"},
			...
			"Luton": {"background-color": "#E6552F","color": "white","accent":"#22355A"},
			"Man City": {"background-color": "#A1C2E7","color": "#061736","accent":"white"},
			"Man Utd": {"background-color": "#D1443F","color": "black","accent":"white"},
			"Newcastle": {"background-color": "WhiteSmoke","color": "black","accent":"#BEA679"},
			"Norwich": {"background-color": "limegreen","color": "yellow"},
			"Nott'm Forest": {"background-color": "#DF4148","color": "white","accent":"black"},
			"Sheffield Utd": {"background-color": "white","color": "#EA4D43","accent":"black"},
			"Southampton": {"background-color": "white","color": "FireBrick","accent":"black"},
			"Spurs": {"background-color": "white","color": "#212935","accent":"#E1E752"},
			"Watford": {"background-color": "black","color": "red"},
			"West Ham": {"background-color": "#7A2E3D","color": "#95CDFA","accent":"white"},
			"Wolves": {"background-color": "#F2BC44","color": "black","accent":"white"},
	...
	def parse_bootstrap(self,last_season=False):
		if not last_season:
			...
			self._prev_bootstrapjson = self.request(url+'bootstrap-static/',last_season=True)
			...
			self._prev_element_types = pd.DataFrame(self._prev_bootstrapjson['element_types'])
			...
			self._prev_teams = []
			for id,name,shortname in zip(self._prev_teamdata['id'],self._prev_teamdata['name'],self._prev_teamdata['short_name']):
	...
	def _teams(self):
		if self.__teams is None:
			self.__teams = []
			total_gc = 0
			for id,name,shortname in zip(self._teamdata['id'],self._teamdata['name'],self._teamdata['short_name']):
				team = Team(id,name,self,shortname=shortname)
	...
	def authenticate(self):
		...
		if not self._authenticated:
			...
			headers = {'authority': 'users.premierleague.com',
				'method': 'GET',
				'path': '/',
				'scheme': 'https',
			...
			if "<Response [403]>" in str(response):
				print("https://mkyong.com/computer-tips/how-to-view-http-headers-in-google-chrome/")
				mout.errorOut("Could not authenticate. Refresh the headers? (datadome cookie)",code=403,fatal=True)
				self._authenticated = False
				return
	...
	def get_player_name(self,player_id):
		for i,id in enumerate(self.elements['id']):
			if id == player_id:
				player_id = i
				break
		return self.elements['first_name'][player_id]+" "+self.elements['web_name'][player_id]
	...
	def get_team_fixtures(self,team_id):
		...
		for event,team_a,team_h,team_a_score,team_h_score,started,finished in zip(self.fixtures['event'],self.fixtures['team_a'],self.fixtures['team_h'],self.fixtures['team_a_score'],self.fixtures['team_h_score'],self.fixtures['started'],self.fixtures['finished']):
			if int(team_id) == int(team_a) or int(team_id) == int(team_h):
				fix.append(dict(event=event,team_a=team_a,team_h=team_h,team_a_score=team_a_score,team_h_score=team_h_score,started=started,finished=finished))
	...
	def get_manager_transfers(self,id):
		# try:
		json = self.request(f'https://fantasy.premierleague.com/api/entry/{id}/transfers/')
		# print(id,json)
		return list(json)
		# except:
		# 	return []
	...
	def get_manager_team(self,id,gw=None,authenticate=True):
		...
		if authenticate:
			...
			this_url = f"https://fantasy.premierleague.com/api/entry/{id}/event/{gw}/picks/"
	...
	def get_fixture_diff(self,opp_team,own_team,is_home,is_attacker,overall=False):
		...
		# overall difference
		return self._teams[own_team-1].strength(overall=overall,defence=not is_attacker,is_home=is_home) - self._teams[opp_team-1].strength(overall=overall,defence=is_attacker,is_home=not is_home)
	...
	def scrape_team_kits(self):
		...
		while True:
			print(i, count)
			try:
				urllib.request.urlretrieve(f"{url}shirt_{i+1}-66.webp", f"kits/{i}.webp")
				i += 1
				count += 1
		...
		for i,name in self._scrape_team_pairs:
			try:
				...
				urllib.request.urlretrieve(f"{url}shirt_{i+1}_1-66.webp", f"kits/{i}_gkp.webp")
	...
	def webp2png(self,path,new,size=87):
		from PIL import Image
		# im = Image.open(path).convert("RGB")
		im = Image.open(path).convert('RGBA').convert('P', palette=Image.ADAPTIVE, colors=255)
		im.thumbnail((size,size), Image.ANTIALIAS)
		im.save(new,"png")
