...
class Manager():
	...
	def is_dead(self):
		if len(self._history_current)<5:
			return False
		return not any([len(self.get_gw_transfers(gw)) > 0 for gw in range(max(self._api._current_gw-4,2),self._api._current_gw+1)])
		...
		for match in league_matches:
			...
			if match['is_bye']:
				...
				if opponent.id == self.id:
					mout.warningOut(f"{self}'s GW{match['event']} oppenent is themselves!")
					continue
	...
	def get_stats(self):
		...
		if len(self._history_current) != 0:
			...
			self._event_transfers_cost = list(self._history_current['event_transfers_cost'])
		...
		for chip in self._history_chips:
			...
			if chip['name'] == '3xc':
				...
				if self._wc1_week is None:
					...
					cutoff = min(min(self._api._wc_cutoff,self._wc1_week+5),self._api._current_gw)
					rel_gw1 = self.active_gws.index(self._wc1_week-1)
					rel_gw2 = self.active_gws.index(cutoff)
					self._wc1_ordelta = self._overall_rank[rel_gw1] - self._overall_rank[rel_gw2]
					self._wc1_ordelta_percent = 100*self._wc1_ordelta/self._overall_rank[rel_gw1]
					...
					rel_gw2 = self.active_gws.index(cutoff)
					self._wc2_ordelta = self._overall_rank[rel_gw1] - self._overall_rank[rel_gw2]
					self._wc2_ordelta_percent = 100*self._wc2_ordelta/self._overall_rank[rel_gw1]

			else:
				print("Unrecognised chip: "+chip['name'])

		self._chip_dict = dict(tc=self._tc_week,bb=self._bb_week,fh=self._fh_week,wc1=self._wc1_week,wc2=self._wc2_week)
		self._chip_names = dict(tc="Triple Captain",bb="Bench Boost",fh="Free Hit",wc1="First Wildcard",wc2="Second Wildcard")

	@property
	def _bb_ptsgain(self):
		if self.__bb_ptsgain is None:
			self.get_current_squad(gw=self._bb_week,force=True)
			self.__bb_ptsgain = sum([p.get_event_score(gw=self._bb_week,not_playing_is_none=False) for p in self._squad.players[-4:]])
			self._squad = None 
	...
	def gw_rank_gain(self):
		try:
			...
			return 100*(self._overall_rank[last_gw] - self._overall_rank[this_gw])/self._overall_rank[last_gw]
		except ValueError:
			return 0
	...
	def get_current_squad(self,gw=None,force=False):
		if self._squad is None or force:
			...
			response = self._api.get_manager_team(self._id,gw=gw,authenticate=self._authenticate)
	...
	def get_squad_history(self):
		...
		all_players = list(set(sum([squad.players for squad in gw_squads.values()],[])))
		...
		for p in all_players:
			...
			p._avg_pts_owned = round(p._points_while_owned/len(owned),1) if owned else None
			p._avg_pts_started = round(p._points_while_started/len(started),1) if started else None
			p._avg_pts_benched = round(p._points_while_benched/len(benched),1) if benched else None
			p._avg_pts_captained = round(p._points_while_captained/len(captained),1) if captained else None
			p._avg_pts_total = round(p.total_points/p.appearances,1) if p.appearances else None

		all_players = sorted(all_players,key=lambda x: x._num_weeks_owned,reverse=True)
	...
	def avg_selection(self):
		return sum([p.selected_by for p in self.players if p.multiplier > 0])/len(self.players)
	...
	def num_nonwc_transfers(self):
		if self._nonwc_transfers is None:
			self._nonwc_transfers = len([t for t in self.transfer_history if t['event'] != self._wc1_week and t['event'] != self._wc2_week and t['event'] != 17 and t['event'] != 7 and t['event'] != self._fh_week])
	...
	def get_event_performed_xpts(self,gw):
		...
		return float(sum([p.get_performed_xpts(gw)*p.multiplier for p in self.players]))
	...
	def create_rank_graph(self,plot=True,show=False):
		...
		if plot:
			...
			with plt.style.context('seaborn'):
				...
				ax.grid(which='both', axis='both',zorder=-1,color='white',linewidth=2)

				# plots

				x = [i for i in self.active_gws if i not in self._api._skip_gws]

				y = [r for i,r in zip(self.active_gws,self._event_rank) if i not in self._api._skip_gws]
				ax.scatter(x,y,label="GW Rank",marker='o',color="r",zorder=2)
				
				y = [r for i,r in zip(self.active_gws,self._overall_rank) if i not in self._api._skip_gws]
				ax.plot(x,y,linestyle='-',marker=None,color="b",label="Overall Rank",zorder=1)
		...
		if plot:
			...
			with plt.style.context('seaborn'):
				...
				if len(self._leagues) > 0:
					...
					mout.errorOut(f"Manager {self.name} ({self.id}) has no league data.")
	...
	def create_points_graph(self,plot=True,show=False):
		...
		if plot:
			...
			with plt.style.context('seaborn'):
				...
				y = [p if i not in self._api._skip_gws else None for i,p in zip(range(1,self._api._current_gw+1),avgs)]
				ax.plot(range(1,self._api._current_gw+1),y,linestyle='-',marker=None,color="b",label="Global Average",zorder=3)

				y = [p for i,p in zip(self.active_gws,pts) if i not in self._api._skip_gws]
				ax.scatter(x,y,label="GW Points",marker='o',color="r",zorder=4)

				hit_cost = [self.get_transfer_cost(w) for w in self.active_gws if w not in self._api._skip_gws]
				hit_yerrs = [[ h for h in hit_cost],[ 0 for h in hit_cost]]
				ax.errorbar(x,y,yerr=hit_yerrs,label="Transfer Cost",marker='o',color="k",zorder=2,linewidth=0,elinewidth=2,fmt='none',capsize=3,capthick=2)
	...
	def make_autosubs(self):
		...
		if len(bench) != 4:
			if self._bb_week != self._api._current_gw:
				mout.warningOut(f'Bench has wrong number of players ({self.name}). And no BB played!')
				print("before:",bench)
		...
		for player in self.players:
			...
			if player.needs_autosub:
				...
				if n_defenders == 3 and player.position_id == 2 and replacement.position_id != 2:
